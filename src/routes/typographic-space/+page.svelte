<main>
	...
	<p>
		...
		is called tracking. If you're setting type at small sizes, like nine points or below, adding a
		little bit of tracking, say 10 or 20 units, can help make the letters more clearly legible.
	</p>
	<p>
		But at large sizes, too much tracking will make a word start to visually disintegrate, because
		the letters aren't close enough to one another to stick together as a word. In fact, if you're
		setting a large title, you may want to set negative tracking, to bring the letters even closer
		together than their native letter spacing.
	</p>
	<p>
	...
	<p>
		Kerning is sometimes necessary when you have two letters that fit together awkwardly. In this
		word, because of the shapes of the uppercase A and the v, there's some distracting extra space
		between these two letters. So we'll kern them to be closer together. Typefaces these days are
	...
	<p>
		So how does line length affect leading? Well, if your lines are longer, you may need more space
		between them in order to help the reader's eyes follow along them. A final variable is text
		alignment. When text is aligned on the left side and not on the right, it's referred to as left
		aligned.
	</p>
	<p>
		The advantage of setting type this way is that it allows the typeface's native letter spacing
		and word spacing to be preserved unaltered. The disadvantage is that it creates an uneven edge
		along one side of the text, called a rag. And in a future video we'll look at how this rag can
		be manipulated to make it as visually pleasing as possible.
	</p>
	<p>
		An alternative to left aligned text is justified text, text that has been aligned on both the
		left and right sides. The advantage of setting type like this is that it creates even edges on
		both sides.
	</p>
	<p>
		But the disadvantages that word spacing and/or tracking have to be altered in order to make
		longer lines shorter and shorter lines longer. And this breaks up the lines of text, creating
		vertical channels through the paragraph, and has to be compensated for by increasing leading.
	</p>
	<p>
